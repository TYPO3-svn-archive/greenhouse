 ...
 */
/**
 * EXAMPLE for using the greenhouse-class for exporting stuff:
 *
 * 		// Create and initialize:
 * 	$this->export = t3lib_div::makeInstance('tx_greenhouse');
 * 	$this->export->init();
...
class tx_greenhouse {
	...
	var $maxFileSize = 3145728;		// 1MB max file size
	var $maxRecordSize = 100000000;	// 1MB max record size
	var $maxExportSize = 1000000000;	// 10MB max export size
...
#debug($vR);
	...
	function export_addFilesFromRelations()	{
            ...
            foreach($this->dat['records'] as $key => $value){
                ...
                $settings = t3lib_div::_GP('tx_greenhouse');
                $patterns = $settings["files"];
                foreach(explode("\n",$patterns) as $pattern){
                    #echo $pattern."<br />";
                    #echo PATH_site.$pattern."<br />";
                    ...
                    foreach($paths as $path){
                        ...
                        if(is_dir($path)){
                            ...
                            /*
                            $this->dat["records"]{$key}["rels"]["fileref"]["softrefs"]["keys"]["typolink"][md5($file).":".$x++] = array(
                                "matchString" => $file,
                                "subst" => array(
                                    "filename" => basename($file),
                                    "ID" => md5($file),
                                ...
                                "tokenizedContent" => "softref:".md5($file)
                            );
                            */
                        }
                    }
                ...
                print_r($this->dat["records"]{$key}["rels"]["fileref"]["softrefs"]["keys"]["typolink"]);
...
#debug($this->import_mode[$table.':'.$uid],$table.':'.$uid);
				...
				$this->import_data[$table][$ID]['tx_greenhouse_origuid'] = $this->import_data[$table][$ID]['uid'];
...
#$fileProcObj->mounts['1f390e42e1dc46f125310ead30c7bd9d']['path'] = '/var/www/typo3/dev/testsite-3.6.0/fileadmin/user_upload/';
		...
		if (is_array($this->remainHeader))	{
			...
			if (is_array($this->dat['header']['pagetree']))	{
				...
				<tr class="bgColor5 tableheader">
					<td>'.$LANG->getLL('greenhousecore_displaycon_controls',1).'</td>
					<td>'.$LANG->getLL('greenhousecore_displaycon_title',1).'</td>
					<td>'.$LANG->getLL('greenhousecore_displaycon_size',1).'</td>
					<td>'.$LANG->getLL('greenhousecore_displaycon_message',1).'</td>
					'.($this->update ? '<td>'.$LANG->getLL('greenhousecore_displaycon_updateMode',1).'</td>' : '').'
					'.($this->update ? '<td>'.$LANG->getLL('greenhousecore_displaycon_currentPath',1).'</td>' : '').'
					'.($this->showDiff ? '<td>'.$LANG->getLL('greenhousecore_displaycon_result',1).'</td>' : '').'
				</tr>';
				...
				$out = '
					<strong>'.$LANG->getLL('greenhousecore_displaycon_insidePagetree',1).'</strong>
					<br /><br />
			...
			if (is_array($this->remainHeader['records']))	{
				...
				if (count($lines))	{
					...
					$out.= '
						<strong>'.$LANG->getLL('greenhousecore_singlereco_outsidePagetree',1).'</strong>
						<br /><br />
						<table border="0" cellpadding="0" cellspacing="1">'.implode('',$rows).'</table>';
				}
			}
	...
	function singleRecordLines($table,$uid,&$lines,$preCode,$checkImportInPidRecord=0)	{
		...
		if ($table==='_SOFTREF_')	{	// Unknown table name:
			...
			$pInfo['title'] = '<em>'.$LANG->getLL('greenhousecore_singlereco_softReferencesFiles',1).'</em>';
			...
			if (is_array($this->display_import_pid_record))	{
				...
				if ($this->update)	{
					...
					$pInfo['updatePath']= $recInf ? htmlspecialchars($this->getRecordPath($recInf['pid'])) : '<b>NEW!</b>';
						...
						// Mode selector:
					$optValues = array();
					$optValues[] = $recInf ? $LANG->getLL('greenhousecore_singlereco_update') : $LANG->getLL('greenhousecore_singlereco_insert');
					if ($recInf) $optValues['as_new'] = $LANG->getLL('greenhousecore_singlereco_importAsNew');
					if ($recInf) {
						if (!$this->global_ignore_pid)	{
							$optValues['ignore_pid'] = $LANG->getLL('greenhousecore_singlereco_ignorePid');
						} else {
							$optValues['respect_pid'] = $LANG->getLL('greenhousecore_singlereco_respectPid');
						}
					}
					if (!$recInf && $GLOBALS['BE_USER']->isAdmin()) $optValues['force_uid'] = sprintf($LANG->getLL('greenhousecore_singlereco_forceUidSAdmin'),$uid);
					$optValues['exclude'] = $LANG->getLL('greenhousecore_singlereco_exclude');

					$pInfo['updateMode'] = $this->renderSelectBox('tx_greenhouse[import_mode]['.$table.':'.$uid.']',$this->import_mode[$table.':'.$uid],$optValues);
				}
			...
			foreach($record['softrefs'] as $info)	{
				...
				if ($info['subst']['type'])	{
					if (strlen($info['subst']['title']))	{
						$pInfo['title'].= '<br/>'.$preCode_B.'<b>'.$LANG->getLL('greenhousecore_singlereco_title',1).'</b> '.htmlspecialchars(t3lib_div::fixed_lgd_cs($info['subst']['title'],60));
					}
					if (strlen($info['subst']['description']))	{
						$pInfo['title'].= '<br/>'.$preCode_B.'<b>'.$LANG->getLL('greenhousecore_singlereco_descr',1).'</b> '.htmlspecialchars(t3lib_div::fixed_lgd_cs($info['subst']['description'],60));
					}
					$pInfo['title'].= '<br/>'.$preCode_B.
											($info['subst']['type'] == 'file' ? $LANG->getLL('greenhousecore_singlereco_filename',1).' <b>'.$info['subst']['relFileName'].'</b>' : '').
											($info['subst']['type'] == 'string' ? $LANG->getLL('greenhousecore_singlereco_value',1).' <b>'.$info['subst']['tokenValue'].'</b>' : '').
											($info['subst']['type'] == 'db' ? $LANG->getLL('greenhousecore_softrefsel_record',1).' <b>'.$info['subst']['recordRef'].'</b>' : '');
				}
				$pInfo['ref'] = 'SOFTREF';
				$pInfo['size'] = '';
...
#debug($info);
	...
	function renderControls($r)	{
		...
		if ($this->mode==='export')	{
			return ($r['type']=='record' ? '<input type="checkbox" name="tx_greenhouse[exclude]['.$r['ref'].']" id="checkExclude'.$r['ref'].'" value="1" /> <label for="checkExclude'.$r['ref'].'">'.$LANG->getLL('greenhousecore_singlereco_exclude',1).'</label>' :
			...
			if ($r['type']=='softref' && is_array($r['_softRefInfo']['subst']) && $r['_softRefInfo']['subst']['tokenID'])	{
				...
				if ($cfg['mode'] === 'editable')	{
					return
						...
						<input type="text" name="tx_greenhouse[softrefInputValues]['.$tokenID.']" value="'.htmlspecialchars(isset($this->softrefInputValues[$tokenID]) ? $this->softrefInputValues[$tokenID] : $cfg['defValue']).'" />';
				}
			}
		}
	}
	...
	function softrefSelector($cfg) {
		...
		if (is_array($cfg['subst']) && $cfg['subst']['tokenID'] && !$fI['RTE_ORIG_ID'])	{
			...
			$optValues = array();
			$optValues[''] = '';
			$optValues['editable'] = $LANG->getLL('greenhousecore_softrefsel_editable');
			$optValues['exclude'] = $LANG->getLL('greenhousecore_softrefsel_exclude');

				// Get current value:
			$value = $this->softrefCfg[$cfg['subst']['tokenID']]['mode'];

				// Render options selector:
			$selectorbox = $this->renderSelectBox('tx_greenhouse[softrefCfg]['.$cfg['subst']['tokenID'].'][mode]',$value,$optValues).'<br/>';
			...
			if ($value === 'editable')	{
				...
				if (strlen($cfg['subst']['title']))	{
					$descriptionField.= '
					<input type="hidden" name="tx_greenhouse[softrefCfg]['.$cfg['subst']['tokenID'].'][title]" value="'.htmlspecialchars($cfg['subst']['title']).'" />
				...
				if (!strlen($cfg['subst']['description']))	{
					$descriptionField.= '
					'.$LANG->getLL('greenhousecore_printerror_description',1).'<br/>
					<input type="text" name="tx_greenhouse[softrefCfg]['.$cfg['subst']['tokenID'].'][description]" value="'.htmlspecialchars($this->softrefCfg[$cfg['subst']['tokenID']]['description']).'" />';
				} else {
					$descriptionField.= '

					<input type="hidden" name="tx_greenhouse[softrefCfg]['.$cfg['subst']['tokenID'].'][description]" value="'.htmlspecialchars($cfg['subst']['description']).'" />'.
					htmlspecialchars($cfg['subst']['description']);
				}

					// Default Value:
				$descriptionField.= '<input type="hidden" name="tx_greenhouse[softrefCfg]['.$cfg['subst']['tokenID'].'][defValue]" value="'.htmlspecialchars($cfg['subst']['tokenValue']).'" />';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/greenhouse/class.tx_greenhouse.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/greenhouse/class.tx_greenhouse.php']);
}
?>
